...
if __name__ == '__main__':
    ...
    file_num = len(files)
    publish_datetimes = generate_schedule_time_next_day(total_videos=file_num, videos_per_day=1, daily_times=[16], start_days=0)
    cookie_setup = asyncio.run(weixin_setup(account_file, handle=True))
    category = TencentZoneTypes.ANIMALS.value  # 标记原创需要否则不需要传
